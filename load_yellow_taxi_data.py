...
def download_file(params):
    colour, year, month = params
    url = f"{BASE_URL}/{colour}/{colour}_tripdata_{year}-{month}.csv.gz"
    file_path = os.path.join(DOWNLOAD_DIR, f"{colour}_tripdata_{year}-{month}.csv.gz")
...
if __name__ == "__main__":
    ...
    # loop through the parameters and download files in parallel
    download_params = []
    for colour in COLOUR:
        for year in YEAR:
            for month in MONTHS:
                download_params.append((colour, year, month))
